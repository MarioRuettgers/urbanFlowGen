...
urbanFlowGen is an open-source Python library for generating synthetic urban Computational Fluid Dynamics (CFD) scenarios tailored for machine learning applications. It provides a fully automated pipeline for:
    ...
    Sampling physically realistic atmospheric inflow conditions from reanalysis data,

    Generating randomized 3D urban geometries with customizable features,

    Creating property files for generating computational meshes using open-source grid generation tools,

    Creating property files for setting up CFD simulations for both CPU- and GPU-based solvers (e.g., OpenFOAM, m-AIA).
