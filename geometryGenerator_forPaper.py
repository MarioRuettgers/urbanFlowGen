...
tower_min_height, tower_max_height = 150, 200 # Range of tower height
...
# --- 5. Generate buildings with controlled types ---
num_buildings = 16
required_types = ['circle', 'tower', 'extreme']
remaining = num_buildings - len(required_types)
building_types = required_types + ['regular'] * remaining
random.shuffle(building_types)  # Shuffle to randomize order
...
while generated < num_buildings and attempts < max_attempts:
    attempts += 1
    btype = building_types[generated]

    # Select building type
    if btype == 'circle':
        diameter = np.random.uniform(min_size, max_size)
        height = np.random.uniform(50, 80)
        w, h = diameter, diameter
        shape = 'circle'
        ...
        for _ in range(100):
            ...
            continue  # Retry tower

    elif btype == 'extreme':
        for _ in range(100):
            w = np.random.uniform(min_size, max_size)
            h = np.random.uniform(min_size, max_size)
            if max(w / h, h / w) <= extreme_aspect_ratio and max(w / h, h / w) > normal_aspect_ratio:
                height = np.random.uniform(5, 80)
                shape = 'rect'
                break
        else:
            continue  # Retry extreme

    else:  # regular
        for _ in range(100):
            w = np.random.uniform(min_size, max_size)
            h = np.random.uniform(min_size, max_size)
            if max(w / h, h / w) <= normal_aspect_ratio:
                height = np.random.uniform(5, 80)
                shape = 'rect'
                break
        else:
            continue  # Retry regular
    ...
    # Create geometry for collision check
    if shape == 'circle':
        radius = w / 2
